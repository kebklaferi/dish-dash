...
/**
 * Structured logger for PaymentService
 * Format: <timestamp> <LogType> <URL> <CorrelationId> <serviceName> - <message>
 */
class Logger {
  private readonly serviceName: string = 'PaymentService';
  ...
  private async publishToRabbitMQ(logMessage: LogMessage, level: LogLevel): Promise<void> {
    ...
    try {
      // Generate routing key based on level: service.level (e.g., paymentservice.info)
